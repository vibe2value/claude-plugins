...
That is the common case. Almost nobody stops to shape before they build. They build, then only later start wondering whether the thing is real. At that point the framework is still the right tool, but the decisions are already behind them. What it needs is evidence.
