# shape-build-launch-run

A Claude Code plugin that guides building software with AI through the vibe2value shape+build+launch+run framework.
...
```
/shape-build-launch-run:guide
```

You can also just say "use the shape-build-launch-run skill", or add a line to your project's `CLAUDE.md` so it stays in play across a whole session.
